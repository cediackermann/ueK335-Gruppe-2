...
import { RootStackParamList, Book as BookType, Publisher as PublisherType } from '../types'; // Stelle sicher, dass der Pfad korrekt ist
...
const Home: React.FC<Props> = ({ navigation }) => {
  ...
  const renderBookRecommendation = () => {
    if (isLoadingBooks || isLoadingPublishers) {
      return <ActivityIndicator size="small" color="#0A2543" style={styles.loadingIndicator} />;
    }

    if (booksError || publishersError) {
      return <Text style={styles.errorText}>Could not load book recommendation.</Text>;
    }

    if (!todaysBook) {
      return <Text style={styles.infoText}>No books available for recommendation.</Text>;
    }
    ...
    return (
      <Card style={styles.card}>
        <Card.Content>
          ...
          <Paragraph style={styles.authorText}>Publisher: {todaysBook.author}</Paragraph>
        </Card.Content>
      </Card>
    );
  };
...
const styles = StyleSheet.create({
  ...
  bookName: { fontSize: 17, fontWeight: '600', color: '#0A2543', flexShrink: 1 },
